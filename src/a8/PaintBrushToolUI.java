...
public class PaintBrushToolUI extends JPanel implements ChangeListener {
	...
	public PaintBrushToolUI() {
		...
		JLabel size_label = new JLabel("size of the brush: ");
		size_panel.setLayout(new BorderLayout());
		size_panel.add(size_label, BorderLayout.WEST);
		s_slider = new JSlider(0,10);
		s_slider.setMajorTickSpacing(1);
		s_slider.setPaintTicks(true);
		s_slider.setPaintLabels(true);
		s_slider.addChangeListener(this);
		size_panel.add(s_slider, BorderLayout.CENTER);
		
		JPanel o_panel = new JPanel();
		JLabel o_label = new JLabel("Opacity: ");
		o_panel.setLayout(new BorderLayout());
		o_panel.add(o_label, BorderLayout.WEST);
		o_slider = new JSlider(0,100);
		o_slider.setMajorTickSpacing(10);
		o_slider.setPaintTicks(true);
		o_slider.setPaintLabels(true);
		o_slider.addChangeListener(this);
		o_panel.add(o_slider, BorderLayout.CENTER);
		
		JPanel function = new JPanel();
		function.setLayout(new GridLayout(0, 1));
		function.add(size_panel);
		function.add(o_panel);
	...
	public JSlider getOpacitySlider(){
		return this.o_slider;
	}
	
	public JSlider getSizeSlider(){
		return this.s_slider;
	}
